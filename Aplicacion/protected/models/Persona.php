...
class Persona extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('cedula, id_departamento', 'numerical', 'integerOnly'=>true),
			array('nombre, apellido, correo', 'length', 'max'=>50),
			array('telefono', 'length', 'max'=>12),
			array('direccion_ip', 'length', 'max'=>15),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre, apellido, correo, telefono, cedula, id_departamento, direccion_ip', 'safe', 'on'=>'search'),
		);
	}
